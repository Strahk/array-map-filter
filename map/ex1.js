...
function getStringsLength(strings) {
  const length = strings.map(string => `${string} contains ${string.length} characters`)
  return length
}

// function getStringsLength(strings) {
//   const length = strings.map(strings => strings.length)
//   return `${strings} contains ${length} characters`
// }
